...
Our empirical results confirm this theoretical expectation: the all-equal configuration ($\lambda_{\text{self}} = \lambda_{\text{welfare}} = \lambda_{\text{fair}} = 1.0$) was the worst performer, while fair-only ($\lambda_{\text{fair}} = 1.0$) achieved the highest agreement rates and most balanced outcomes (see Section~\ref{sec:results}).
...
\textcolor{gray}{[PLACEHOLDER: Additional $\lambda$ combinations if available, e.g., $\lambda_{\text{self}}=0.7$, $\lambda_{\text{fair}}=0.3$ as recommended by the literature review on reward weighting.]}
...
\textbf{All-equal performs worst} (agreed/ratio\_self $= 0.423$, ratio\_nash $= 0.386$). This confirms \textbf{H3}: the welfare term provides no learning signal in distributive games ($\sim$31\% of the training set), effectively wasting one-third of the gradient budget. The all-equal configuration has no theoretical justification---equal weighting does not correspond to any established social welfare function.
...
For context, Table~\ref{tab:luca_results} shows results from Franceschetti's evaluation framework on Llama-3.1-8B with competitive reward ($U_A$ only):

\begin{table}[H]
\centering
\caption{Baseline results from \citet{franceschetti2025advantage}'s evaluation (Llama-3.1-8B, competitive reward).}
\label{tab:luca_results}
\begin{tabular}{lccc}
\toprule
\textbf{Method} & \textbf{Best Step} & \textbf{Mean Reward} & \textbf{Agreement Rate} \\
\midrule
DPO & 2000 & 58.6 & 89.2\% \\
REFUEL & 4000 & 78.0 & 90.8\% \\
GRPO (onesided) & 400 & 76.0 & 90.8\% \\
GRPO turn-level & 1200 & 76.1 & 93.2\% \\
\bottomrule
\end{tabular}
\end{table}

\textcolor{gray}{[PLACEHOLDER: Own evaluation on the same framework with Qwen3-14B models --- self-only, fair-only, all-equal on held-out test games. Table with mean\_reward, std, agreement\_rate analogous to Franceschetti's results.]}

The simplified example in Table~\ref{tab:evaluation_example} illustrates the expected pattern of results in a positive scenario, where the cooperative model achieves both higher individual reward and substantially higher joint welfare and Nash product.

\begin{table}[H]
\centering
\caption{Simplified illustrative example of expected evaluation results. Mean negotiation outcomes across models on a mixed-motive multi-issue setup. Each value represents the average over 200 negotiation episodes.
$R_A$ and $R_B$ denote the individual agent rewards; 
$W = R_A + R_B$ measures total welfare (efficiency), and 
$N = R_A \times R_B$ measures fairness and cooperation.}
\vspace{0.3cm}
\label{tab:evaluation_example}
\renewcommand{\arraystretch}{1.15}
\begin{tabular}{lcccc}
\toprule
\textbf{Model} & $\mathbf{R_A}$ & $\mathbf{R_B}$ & $\mathbf{W = R_A + R_B}$ & $\mathbf{N = R_A \times R_B}$ \\
\midrule
Base (Qwen3-14B)       & 45.3 & 40.7 & 86.0  & 1\,844 \\
GRPO (zero-sum)       & 52.4 & 37.9 & 90.3  & 1\,986 \\
LA-GRPO (zero-sum)     & \textbf{54.6} & 36.2 & 90.8  & 1\,977\\
\textbf{LA-GRPO (ours)} & \textbf{56.8} & \textbf{49.1} & \textbf{105.9} & \textbf{2\,789} \\
\bottomrule
\end{tabular}
\end{table}
...
\paragraph{All-equal shows almost no change,} consistent with its weak training performance---the model did not learn a sufficiently strong cooperative signal to transfer.
...
\paragraph{RQ2 (Reward Balance).} Clear evidence supports the hypothesis that the Nash product ($\lambda_{\text{fair}}$) is the most effective cooperative reward component. Fair-only outperforms self-only on agreement rate, welfare, and Nash product, while all-equal underperforms both. This aligns with \citet{caragiannis2019unreasonable}'s theoretical prediction and our hypotheses H1/H3. The welfare term is redundant in distributive games and harmful when given equal weight to the other components.
...
\begin{enumerate}
    ...
    \item \textbf{$\lambda$ ablation study} demonstrating that the Nash product ($\lambda_{\text{fair}}$) is the most effective cooperative term---it simultaneously increases agreement rate and outcome fairness, consistent with theoretical predictions from \citet{Nash1950THEBP} and \citet{caragiannis2019unreasonable}.
