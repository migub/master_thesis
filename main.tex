...
\paragraph{Linear Scalarization Limits.} \citet{hayes2022practical} established that linear scalarization can only discover solutions on the convex hull of the Pareto front. Our reward formula partially circumvents this limitation because the Nash product term ($U_A \times U_B$) is inherently non-linear.
...
We compute $R_{\text{coop}}$ using game-normalized ratios instead of raw values:
...
Having defined the cooperative reward $R_{\text{coop}}$, we can now compute the local advantage signal used by LA-GRPO to assign credit and update the policy.
\par
To isolate the learning signal for a particular dialogue turn, we follow \citeauthor{franceschetti2025advantage}'s (\citeyear{franceschetti2025advantage}) Local Advantage Group Relative Policy Optimization procedure. A turn $h$ is sampled from a geometric distribution $h \sim \mathrm{Geom}(0.3)$, bounded by $\max\_rounds - 1$. Rounds $0 \ldots h-1$ are played once as a shared prefix, identical across all $G$ generations. From round $h$ onward, $G$ independent continuations are generated.
...
Based on the theoretical framework and reward design, we formulate the following hypotheses:
...
\paragraph{Extended Game Set.} We extend Franceschetti's game set from 3 scenarios, 10 issues, and 23 game combinations to \textbf{5 scenarios, 19 issues, and 47 game configurations} (19 single-issue and 28 two-issue, with one conflicting combination excluded):
...
\midrule
Generic Rental Agreement & 4 issues (rent, duration, deposit, maintenance) & Franceschetti \\
Generic Loan Agreement & 3 issues (rate, term, collateral) & Franceschetti \\
Generic Merger & 3 issues (valuation, retention, governance) & Franceschetti \\
Joint Venture & 4 issues (R\&D budget, revenue split, data sharing, authority) & New \\
Employment Contract & 5 issues (salary, remote work, training, equity, scope) & New \\
\bottomrule
\end{tabular}
\end{table}
...
We train three configurations plus a baseline to isolate the effect of each reward component:
...
The first three configurations isolate the effect of each reward component individually. All-equal combines all three terms with equal weight. Self-fair-equal tests whether the welfare component can be dropped without loss of performance, giving equal weight to self-interest and fairness only.


\subsection{Technical Setup}

\begin{itemize}
    \item \textbf{GPU:} NVIDIA RTX 5090 (32\,GB VRAM, CUDA 12.8) / A100 (80\,GB)
    \item \textbf{Quantization:} BitsAndBytes 4-bit (nf4, double quant, bf16 compute)
    \item \textbf{Generations per prompt:} $G = 8$
    \item \textbf{Logging:} Weights \& Biases (\texttt{ratio\_rcoop} as primary training metric)
    \item \textbf{Evaluation:} Fixed eval dataset with 10 samples per game type (deterministic)
    \item \textbf{Version control:} Model weights, configuration files, and generated dialogues are version-controlled using GitHub, ensuring reproducibility and transparent documentation of all experiments.
\end{itemize}

\textcolor{gray}{[PLACEHOLDER: Learning rate, batch size, max\_steps, warmup schedule, optimizer details.]}
...
To test generalization, we evaluate on the Trust Game---a 2-turn game-theoretic setting the model was never trained on:
\begin{itemize}
...
Table~\ref{tab:training_dynamics} summarizes the key metrics from the self-only and fair-only training runs at approximately 400 steps, logged via Weights \& Biases.

\begin{table}[H]
\centering
\caption{Training metrics at $\sim$400 steps for self-only vs.\ fair-only configurations.}
\label{tab:training_dynamics}
\begin{tabular}{lcc}
\toprule
\textbf{Metric} & \textbf{Self-Only (step $\sim$400)} & \textbf{Fair-Only (step $\sim$400)} \\
\midrule
ratio\_self & 0.51--0.79 & 0.44--0.75 \\
ratio\_welfare & 0.65--0.81 & 0.63--0.85 \\
ratio\_nash & 0.53--0.64 & 0.41--0.73 \\
Agreement rate & 0.78--1.0 & \textbf{0.95--1.0} \\
KL divergence & 0.07--0.13 & 0.22--0.62 \\
\bottomrule
\end{tabular}
\end{table}
...
\textcolor{gray}{[PLACEHOLDER: Learning curve plots from WandB --- ratio\_rcoop over training steps for all three runs.]}
...
Table~\ref{tab:eval_results} presents the main evaluation results across all five model configurations. Each model was evaluated on 560 games (14 configurations $\times$ 2 starting roles $\times$ 20 repetitions), covering all 7 archetypes with a balanced representation. All games are played in selfplay against the frozen base model opponent, with GPT-4o-mini as outcome judge, temperature $= 1.0$, and seed $= 42$.

\begin{table}[H]
\centering
\caption{Negotiation evaluation results (20 repetitions, 560 games per model). Bold indicates best per column. 95\% bootstrap confidence intervals in brackets.}
...
\begin{table}[H]
\centering
\caption{Agreement rate (\%) by archetype across models.}
...
To evaluate whether cooperative training generalizes beyond the negotiation domain, we test all four model variants on the Trust Game---a canonical game-theoretic setting the models were never trained on.
...
\paragraph{RQ1 (LA-GRPO).} \textcolor{gray}{[PLACEHOLDER: If LA-GRPO runs with cooperative reward exist, compare GRPO vs.\ LA-GRPO. If not, discuss why only GRPO was tested and what is expected from LA-GRPO based on Franceschetti's competitive results.]}
...
\begin{enumerate}
    ...
    \item \textbf{$\lambda$ ablation study} across five reward configurations demonstrating that combining all three objectives ($\lambda_{\text{self}} = \lambda_{\text{welfare}} = \lambda_{\text{fair}} = 1.0$) yields the best overall performance: $+21\%$ on $U_A$, $+14$ percentage points on agreement rate, and $+18\%$ on Nash product over the base model. The Nash product is the strongest individual cooperative signal (fair-only $\gg$ self-only), but dropping the welfare component significantly hurts performance, refuting the initial hypothesis that welfare would be redundant.
...
\begin{itemize}
    ...
    \item \textbf{LA-GRPO not yet trained with cooperative reward:} \textcolor{gray}{[IF APPLICABLE: Only standard GRPO was trained despite LA-GRPO being implemented. RQ1 therefore remains partially answered based on Franceschetti's competitive results rather than direct cooperative evidence.]}
...
\begin{itemize}
    ...
    \item \textbf{LA-GRPO with cooperative reward:} Turn-level credit assignment combined with fairness-oriented rewards should provide more precise learning signal for cooperative behavior---the expected synergy of this thesis's two main ideas.
