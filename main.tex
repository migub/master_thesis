...
To evaluate whether cooperative training generalizes beyond the negotiation domain, all trained model variants are tested on the Trust Game---a canonical game-theoretic setting the models were never trained on. Two conditions are evaluated: each trained model plays against the frozen base model (Tables~\ref{tab:trust_investor} and~\ref{tab:trust_trustee}), and each trained model plays against itself in selfplay (Table~\ref{tab:trust_selfplay}).
...
\begin{enumerate}
    ...
    \item \textbf{$\lambda$ ablation across GRPO and LA-GRPO} reveals distinct roles for each reward component. The \textbf{Nash product} ($\lambda_{\text{fair}}$) is the strongest individual cooperative signal: fair-only significantly outperforms self-only on agreement rate, Nash ratio, and welfare for both GRPO and LA-GRPO. \textbf{Self-interest} ($\lambda_{\text{self}}$) alone improves raw payoff $U_A$ but fails to improve cooperative metrics over baseline. \textbf{Welfare} ($\lambda_{\text{welfare}}$) alone provides a weak training signal with declining Nash ratio, but in combination with the other components it proves essential---removing it (self-fair-equal vs.\ all-equal) significantly hurts performance, refuting hypothesis H3. The \textbf{combined reward} (all-equal, $\lambda_{\text{self}} = \lambda_{\text{welfare}} = \lambda_{\text{fair}} = 1.0$) achieves the best aggregate GRPO performance ($+21\%$ on $U_A$, $+14$ pp agreement rate), with its strongest effect on the hardest archetype: integrative-distributive agreement nearly doubles from $47.5\%$ (base) to $95.0\%$ (all-equal). When combined with LA-GRPO, the \textbf{fair-only configuration achieves the highest per-deal cooperation} of any model (agreed Nash ratio $0.787$), dominating across all archetypes---particularly integrative games ($0.920$ vs.\ $0.715$ for self-only). For single-objective rewards, LA-GRPO substantially outperforms GRPO; for the blended all-equal reward, the methods perform comparably, suggesting that multi-objective rewards already provide sufficient gradient diversity without turn-level decomposition.
