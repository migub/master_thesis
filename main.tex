...
\paragraph{Fair-only wins Nash product on every archetype.} Table~\ref{tab:lagrpo_arch_nash} shows agreed-only Nash ratios by archetype. Fair-only dominates across all game types, with a particularly striking advantage on integrative games ($0.920$ vs.\ $0.715$ for self-only). This indicates that the Nash product reward is particularly effective on integrative games when training is run to convergence.
