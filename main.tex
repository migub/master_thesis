...
\begin{enumerate}[label={}, leftmargin=0pt, itemsep=1.2em]
    ...
    \par This question tests whether applying LA-GRPO at the single-turn level improves negotiation outcomes compared to baseline GRPO. Performance is compared by measuring negotiation outcomes (agreement rate, agent payoff $U_A$, Nash product) at both full convergence and at matched compute budgets to disentangle method quality from training duration.
    ...
    \par This question assesses whether cooperative agents remain effective when facing adversarial opponents. The trained model is evaluated against four adversarial opponent personas representing distinct manipulation strategies---hardball (aggressive concession refusal), deceptive (misrepresenting preferences), anchoring (extreme initial positions), and stubborn (rigid refusal to compromise).
