...
\begin{itemize}
    ...
    \item \textbf{Cooperative training transfers and stays robust.} General capabilities are preserved ($\pm 0.6$ pp on MMLU-Pro, IFEval, GSM8K), behaviour transfers modestly to the Trust Game, and the model substantially outperforms the base against three of four adversarial personas---cooperative training does not produce an exploitable agent.
\end{itemize}
...
\paragraph{Fair-only with LA-GRPO produces the highest deal quality.} The agreed Nash ratio bar chart reveals a different picture than the overall Nash ratio: LA-GRPO fair-only stands out clearly with $0.787$, the highest deal quality of any configuration and the only one substantially above the base model reference ($0.744$). For self-only, the relationship reverses---GRPO self-only ($0.728$) slightly outperforms LA-GRPO self-only ($0.716$), with both below the base model. This suggests LA-GRPO self-only gains higher overall scores mainly by closing \emph{more} deals (95.5\% agreement), but the individual deals it closes are not better than the base model's. For all-equal, GRPO ($0.747$) and LA-GRPO ($0.740$) perform near the base model level. Overall, cooperative training primarily improves the agreement rate; only the Nash-product reward combined with LA-GRPO's turn-level credit assignment also yields better deals when agreements are reached.
...
\begin{enumerate}
    ...
    \item \textbf{Trust Game evaluation} showing modest but measurable transfer of cooperative behavior to an unseen game-theoretic setting. Fair-only as investor shows higher trust (send $5.48$ vs.\ $5.00$) and 31\% higher Nash product ($83.1$ vs.\ $63.4$).
