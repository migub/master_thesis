...
The reward function used in this work draws on three distinct strands of research in cooperative reinforcement learning: prosocial preferences, welfare maximization, and inequity aversion. Each strand has been explored independently in classical multi-agent settings; this thesis combines them and applies them to the LLM negotiation domain.

The earliest and most direct precedent is the work of \citet{peysakhovich2017prosocial}, who trained deep RL agents on social dilemmas using a reward of the form $R_i = (1 - \alpha) \cdot U_{\text{self}} + \alpha \cdot U_{\text{other}}$, demonstrating that a small dose of prosocial preference is sufficient to sustain cooperation in repeated games, even when the partner is novel. This linear blending of self-interest and other-regarding utility is the conceptual template adopted in this thesis, generalized to three components rather than two. \citet{mckee2020social} extended this line of work by showing that the \emph{diversity} of training partners matters more than the specific opponent's level of prosociality---a finding that motivates self-play training as a future direction.

Beyond simple prosociality, two related but distinct cooperative signals have been studied. \textbf{Welfare maximization} encodes the principle that the total value generated by the negotiation matters: \citet{pmlr-v162-liu22l} applied RL techniques to model fairness and efficiency trade-offs in multi-agent economic systems, connecting game-theoretic welfare optimization with policy learning. \textbf{Inequity aversion}, introduced into deep MARL by \citet{hughes2018inequity}, captures the intuition that agents care not only about their own payoff but also about how their outcome compares to others'. Agents with inequity aversion learn to avoid both disadvantageous and advantageous inequality, which leads to more cooperative behavior in intertemporal social dilemmas. The Nash product reward used in this thesis operationalizes both ideas simultaneously: maximizing $U_A \times U_B$ rewards both high total value and balanced distribution, as established in Section~\ref{sec:cooperative_negotiation_theory}.

A practical concern when combining multiple objectives is whether linear scalarization---weighting components and summing them---is expressive enough to capture the full trade-off space. \citet{hayes2022practical} establish that linear scalarization can only discover solutions on the convex hull of the Pareto front, and that non-linear scalarizations are needed to reach concave portions of the front. The reward formula adopted here ($\lambda_s \cdot \text{ratio\_self} + \lambda_w \cdot \text{ratio\_welfare} + \lambda_f \cdot \text{ratio\_nash}$) is a linear combination, but the Nash product term itself is non-linear in the underlying payoffs, partially mitigating this limitation.

The application of these ideas specifically to LLM negotiation is recent. \citet{duan2024gtbenchuncoveringstrategicreasoning} evaluated LLMs' strategic reasoning across diverse game-theoretic environments and identified limitations in long-term planning and strategic adaptation, motivating the use of reinforcement learning rather than pure prompting. \citet{jiang-akcakir-2025-explicit} empirically showed that explicit cooperation cues substantially improve negotiation success and human-likeness in LLM interactions, while \citet{vaccaro2025advancingainegotiationsnew} reported large-scale evidence that agents using cooperative and empathetic language achieve higher joint gains than purely competitive agents. The present work differs from these by encoding cooperation directly in the reward signal during fine-tuning rather than in the system prompt, producing models whose cooperative behavior is intrinsic to their weights.
...
\paragraph{Matched-budget findings.} The picture at matched compute differs substantially from the convergence-point comparison. For \textbf{single-objective rewards} (self-only, fair-only), GRPO outperforms LA-GRPO clearly: Fair-Only GRPO reaches $93.1\%$ agreement vs.\ LA-GRPO's $77.5\%$, a gap of $15.6$ percentage points. For the \textbf{blended all-equal reward}, LA-GRPO retains a small advantage ($+3.1$ pp agreement, $+0.029$ ratio Nash). The combined result suggests that LA-GRPO is \emph{not inherently more sample-efficient}; rather, it needs additional training time to converge for single-objective rewards, while reaching parity (or slight advantage) earlier for multi-objective rewards.

\paragraph{Implication for RQ1.} The two perspectives---convergence-point (Section~\ref{sec:grpo_vs_lagrpo}) and matched-budget (this section)---are complementary rather than contradictory. They answer different questions:
\begin{itemize}
    \item \emph{``Which method produces better negotiators in deployment?''} $\to$ LA-GRPO at its convergence point, particularly for single-objective rewards.
    \item \emph{``Which method is more compute-efficient?''} $\to$ GRPO for single-objective rewards, LA-GRPO for multi-objective rewards.
\end{itemize}
The robust finding across both comparisons is that the \textbf{combination of turn-level credit assignment with the multi-objective all-equal reward} performs consistently well: it leads in the matched-budget comparison and remains competitive at full convergence. This suggests that the value of LA-GRPO lies less in faster learning and more in its ability to refine multi-objective signals over longer training horizons.


\subsection{Comparative Discussion}

\paragraph{RQ1 (LA-GRPO).} Turn-level credit assignment improves cooperative negotiation learning, but the magnitude depends on the reward function and the training budget. At their respective convergence points (Section~\ref{sec:grpo_vs_lagrpo}), LA-GRPO substantially outperforms GRPO for single-objective rewards---self-only achieves $95.5\%$ agreement (vs.\ $79.8\%$ GRPO), and fair-only achieves the highest per-deal Nash ratio of any model ($0.787$ agreed). At matched compute budgets (Section~\ref{sec:matched_budget}), however, GRPO is faster to converge for single-objective rewards, while LA-GRPO retains a small advantage for the blended all-equal reward. The robust finding across both comparisons is that the combination of LA-GRPO with multi-objective rewards is the most consistently effective configuration---suggesting that the value of turn-level credit assignment lies in refining complex reward signals over longer training, rather than in faster learning.
