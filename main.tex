...
\begin{figure}[H]
    ...
    \caption[Agreed game-normalized Nash ratio by $\lambda$ configuration.]{Agreed game-normalized Nash ratio by $\lambda$ configuration, restricted to successful negotiations. The Nash ratio is the achieved Nash product divided by the maximum possible Nash product for that game (in $[0, 1]$). Bars are grouped by reward configuration to enable direct GRPO vs.\ LA-GRPO comparison. The y-axis starts at $0.5$ to emphasize differences.}
    \label{fig:nash_bar}
\end{figure}

\paragraph{Fair-only with LA-GRPO produces the highest deal quality.} The agreed Nash ratio bar chart reveals a different picture than the overall Nash ratio: LA-GRPO fair-only stands out clearly with $0.787$, the highest deal quality of any configuration. For self-only, the relationship reverses---GRPO self-only ($0.728$) slightly outperforms LA-GRPO self-only ($0.716$). This suggests LA-GRPO self-only gains higher overall scores mainly by closing \emph{more} deals (95.5\% agreement), but the individual deals it closes are not better than GRPO's. For all-equal, GRPO ($0.747$) and LA-GRPO ($0.740$) perform comparably. Overall, the fair-only Nash product reward combined with LA-GRPO's turn-level credit assignment yields the most balanced, efficient deals when agreements are reached.
