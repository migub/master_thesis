...
\begin{itemize}
    \item \textbf{Cooperative reward design works.} The combined reward (all-equal) is the strongest GRPO configuration, raising agreement rate from $78.6\%$ (base) to $92.7\%$ and nearly doubling agreement on the hardest archetype (integrative-distributive: $47.5\% \to 95.0\%$).
    \item \textbf{Fairness is the strongest single signal.} The Nash product reward (fair-only) substantially outperforms self-interest alone on every cooperative metric. Pure self-interest improves the agent's own payoff but does not improve cooperation over the untrained baseline.
    \item \textbf{LA-GRPO helps, but the picture is nuanced.} At full convergence, LA-GRPO outperforms GRPO on single-objective rewards (LA-GRPO fair-only achieves the highest per-deal Nash ratio of any tested model). At matched compute budgets, however, GRPO is faster for single-objective rewards while LA-GRPO retains a slight advantage only on the blended reward.
    \item \textbf{Cooperative training transfers and stays robust.} The trained models maintain their general capabilities (MMLU-Pro, IFEval, GSM8K within $\pm 0.6$ pp of the base model). They transfer modestly to the Trust Game and---most importantly---improve substantially against three of four adversarial opponent personas, demonstrating that cooperative training does not produce an exploitable agent.
\end{itemize}
