...
\begin{enumerate}[label=\textbf{H\arabic*:}]
    \item Fair-only ($\lambda_{\text{fair}}=1.0$) achieves higher agreement rate than self-only ($\lambda_{\text{self}}=1.0$), because the Nash product punishes failed negotiations with a reward of 0.
...
\textbf{Fair-only achieves higher agreement rate} (0.95+ vs.\ 0.78+). The Nash product assigns a reward of zero to failed negotiations ($U_A \times U_B = 0$ whenever either party gets nothing), creating a strong incentive to always reach a deal. This is a central result for \textbf{RQ2}: the reward function directly shapes whether the agent learns to reach agreements.
