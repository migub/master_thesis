...
\begin{figure}[H]
    ...
    \caption{Negotiation performance during training across four $\lambda$ configurations. All curves are smoothed with a running average (window = 25 steps). Shaded regions indicate variance. Left: mean agent payoff $U_A$. Center: agreement rate. Right: normalized Nash ratio. All-equal and fair-only show the strongest improvements, while self-only improves $U_A$ but not cooperative metrics.}
