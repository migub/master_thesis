...
Large Language Models (LLMs) are increasingly used to negotiate, advise, and act on behalf of people, but most current models default to either extracting maximum value for one side or following whatever instructions a system prompt happens to contain. This thesis investigates whether cooperative negotiation behaviour can instead be trained directly into an LLM, so that fairness and joint value creation become part of the model's default behaviour rather than a prompt-level instruction.
