...
Figures~\ref{fig:training_performance} and~\ref{fig:training_quality} show the training dynamics for the five GRPO configurations, logged via Weights \& Biases. All curves are smoothed with a centered running average (window $= 100$ data points). GRPO runs were all capped at 600 steps; LA-GRPO runs (Figures~\ref{fig:lagrpo_training_performance} and~\ref{fig:lagrpo_training_quality}) ran longer but use fewer tokens per step due to the shared-prefix mechanism.
...
Figures~\ref{fig:lagrpo_training_performance} and~\ref{fig:lagrpo_training_quality} show the corresponding training curves for the three LA-GRPO configurations, which ran for up to 2000 steps.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{images/lagrpo_training_curves_performance.pdf}
    \caption{LA-GRPO negotiation performance during training across three $\lambda$ configurations (up to 2000 steps). Same smoothing and layout as Figure~\ref{fig:training_performance}. Self-only continues to improve over the full training range, while fair-only (which stopped earlier) reaches high Nash and welfare ratios more quickly.}
...
\textbf{The reward function shapes learning dynamics.} All models reach agreements (non-failed negotiations yield $R_{\text{coop}} > 0$ regardless of $\lambda$), but the configurations differ in which kinds of deals they learn to prefer. Under self-only, the agent can obtain high reward from lopsided outcomes (e.g., $U_A = 80, U_B = 10$), but such aggressive offers tend to be rejected. Under Nash product, only balanced outcomes yield high reward ($50 \times 50 = 2500 \gg 80 \times 10 = 800$), so the agent learns to propose offers the opponent accepts. This mechanism explains why fair-only's agreement rate rises quickly early in training, while self-only's plateaus at a lower level---addressing \textbf{RQ2}.
