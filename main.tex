...
Four $\lambda$ configurations are trained alongside a base-model baseline to isolate the effect of each reward component:
...
Figures~\ref{fig:training_performance} and~\ref{fig:training_quality} show the training dynamics across all four $\lambda$ configurations, logged via Weights \& Biases. All curves are smoothed with a running average (window $= 60$ steps).

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{images/training_curves_performance.pdf}
    \caption{Negotiation performance during training across four $\lambda$ configurations. All curves are smoothed with a running average (window $= 60$ steps). Shaded regions indicate variance. Left: mean agent payoff $U_A$. Center: agreement rate. Right: normalized Nash ratio. All-equal and fair-only show the strongest improvements, while self-only improves $U_A$ but not cooperative metrics.}
...
\begin{figure}[H]
    ...
    \caption{Deal quality during training, computed only over successful negotiations. Left: agreed $U_A$ (excluding failed negotiations). Right: agreed Nash ratio. Self-only achieves the highest individual payoff on agreed deals, while fair-only leads on balanced outcomes.}
