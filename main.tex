...
Large Language Models (LLMs) are increasingly used to negotiate, advise, and act on behalf of people. When cooperative or fair behaviour is desired, it is typically requested through system prompts---instructions like ``be fair'' or ``find a mutually beneficial deal''. This thesis investigates whether such behaviour can instead be trained directly into an LLM, so that fairness and joint value creation become part of the model's defaults rather than a prompt that can be ignored, overridden, or forgotten.

The work extends \citet{franceschetti2025advantage}'s Local Advantage Group Relative Policy Optimization (LA-GRPO) from competitive (zero-sum) to cooperative negotiation. The original self-interest reward is replaced with a scalar combining three components: the agent's own payoff, social welfare ($U_A + U_B$), and the Nash product ($U_A \times U_B$). Each component is game-normalized so that distributive, integrative, and compatible games contribute equally to the gradient. Five reward configurations are trained with GRPO, three of them additionally with LA-GRPO. The base model is Qwen3-14B-Instruct, fine-tuned with LoRA adapters on a single rented GPU.

Models are evaluated on the extended LAMEN environment (5 scenarios, 47 game configurations), on a held-out out-of-sample set, on the Trust Game as an out-of-domain transfer test, against four adversarial opponent personas, and on three capability benchmarks (MMLU-Pro, IFEval, GSM8K).

The main results are:

\begin{itemize}
    \item \textbf{Cooperative reward design works.} The combined reward (all-equal) is the strongest GRPO configuration, raising agreement rate from $78.6\%$ to $92.7\%$ and nearly doubling agreement on the hardest archetype ($47.5\% \to 95.0\%$).
    \item \textbf{Fairness is the strongest single signal.} The Nash product reward (fair-only) clearly outperforms self-interest alone. Pure self-interest improves the agent's own payoff but does not improve cooperation over the baseline.
    \item \textbf{LA-GRPO helps, but the picture is nuanced.} At full convergence, LA-GRPO outperforms GRPO on single-objective rewards (LA-GRPO fair-only achieves the highest per-deal Nash ratio of any tested model). At matched compute budgets, GRPO is faster for single-objective rewards while LA-GRPO retains a slight edge only on the blended reward.
    \item \textbf{Cooperative training transfers and stays robust.} General capabilities are preserved (within $\pm 0.6$ pp of the base model on MMLU-Pro, IFEval, GSM8K), behaviour transfers modestly to the Trust Game, and the cooperative model substantially outperforms the base against three of four adversarial personas---showing that cooperative training does not produce an exploitable agent.
\end{itemize}

The broader contribution is a small step toward \emph{intuitive} moral alignment: cooperative dispositions encoded in a model's weights through reward shaping, rather than instructed at inference time. This matters as LLMs are increasingly deployed in autonomous settings where prompt-level guardrails become less reliable.
