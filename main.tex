...
LA-GRPO training runs with three cooperative reward configurations (self-only, fair-only, all-equal) were conducted and are evaluated in Section~\ref{sec:results}.
...
Five $\lambda$ configurations are trained with GRPO to explore the effect of each reward component. Based on these results, the three most informative configurations are retrained with LA-GRPO. Welfare-only and self-fair-equal are omitted from LA-GRPO training, as the GRPO ablation showed that welfare alone provides a weak signal and that removing the welfare component hurts performance. This also reduces compute costs, since LA-GRPO requires more training steps per run (though fewer tokens per step due to the shared prefix mechanism).

\begin{table}[H]
\centering
\caption{$\lambda$ configurations and training methods. LA-GRPO trains for more steps but consumes fewer tokens per step than GRPO, as the shared dialogue prefix is generated only once per training step.}
...
Welfare-Only & 0.0 & 1.0 & 0.0 & GRPO only & 598 \\
All-Equal & 1.0 & 1.0 & 1.0 & GRPO + LA-GRPO & 620 / 2000 \\
Self-Fair-Equal & 0.5 & 0.0 & 0.5 & GRPO only & 820 \\
\bottomrule
\end{tabular}
\end{table}
...
All results reported in Chapter~\ref{sec:results} are based on post-training evaluation. No evaluation was conducted during training; training progress was monitored via the WandB-logged reward metrics only.
...
Generation uses temperature $= 1.0$ with a maximum of 200 tokens per turn and 5 negotiation rounds per agent. The same generation parameters are applied to all models, including the base model, to ensure a fair comparison.

\subsubsection{Outcome Extraction and Agreement Definition}

After each dialogue, the transcript is sent to GPT-4o-mini, which extracts the final agreement as a structured JSON specifying the agreed value for each negotiated issue. \citet{franceschetti2025advantage} verified that this extraction achieves 98\% accuracy across 500 manually inspected samples.

A negotiation is classified as \textit{agreed} if either agent receives a positive payoff ($U_A > 0$ or $U_B > 0$). This includes partial agreements where agents agree on some but not all issues---a known limitation, as partial agreements can produce positive rewards for incomplete deals, slightly inflating agreement rates and reward metrics.

\subsubsection{Negotiation Outcome Metrics}

For each dialogue, agent-specific utilities $U_A$ and $U_B$ are computed from the payoff tables. From these values, aggregate metrics are derived.

The \textbf{joint welfare} captures total efficiency:
\begin{equation}
W = U_A + U_B.
\end{equation}

The \textbf{Nash product} measures fairness and balance:
\begin{equation}
N = U_A \times U_B.
\end{equation}

All metrics are additionally reported as \textbf{game-normalized ratios} (each in $[0, 1]$), enabling comparison across game types with different payoff scales:
$\text{ratio\_self} = U_A / \max U_A$,
$\text{ratio\_welfare} = W / \max W$,
$\text{ratio\_nash} = N / \max N$, and
$\text{ratio\_rcoop} = R_{\text{coop}} / \max R_{\text{coop}}$.

\textbf{Agreed-only variants} of all metrics are computed exclusively over successful negotiations, separating deal quality from agreement rate. A \textbf{per-archetype breakdown} further splits metrics by game type to reveal whether models perform differently on compatible, distributive, and integrative games.

\subsubsection{Statistical Testing}

Since negotiation payoffs are not normally distributed (failed negotiations produce zeros, creating a bimodal distribution), non-parametric tests are used throughout.
...
\begin{itemize}
    ...
    \item \textbf{Bootstrap confidence intervals:} 95\% CIs are computed via non-parametric bootstrap with 10{,}000 resamples. For each resample, $N$ observations are drawn with replacement and the mean is computed; the 2.5th and 97.5th percentiles form the interval.
\end{itemize}
...
To verify that alignment on negotiation tasks does not harm general capabilities, the best-performing GRPO and LA-GRPO models (both all-equal) are re-evaluated on three standard benchmarks using the Language Model Evaluation Harness \citep{eval-harness} with a vLLM backend:
...
\paragraph{Interpretation.} The negotiation-specific RL training preserves the model's generalist capability profile. This is consistent with the training setup: LoRA adapts only a small fraction of the model's parameters, and KL regularization constrains the policy from diverging too far from the base model. The result supports the claim that cooperative alignment can be trained without imposing a ``capability tax'' on general language understanding, mathematical reasoning, or instruction following. \citet{franceschetti2025advantage} reported a similar finding for competitive LA-GRPO training on LLaMA-3.1-8B, and the present results confirm that this robustness extends to cooperative reward objectives and a larger model.
...
\begin{itemize}
    ...
    \item \textbf{Matched-budget GRPO vs.\ LA-GRPO comparison:} The current comparison uses different step counts, and while LA-GRPO is more token-efficient per step, the total token budgets were not precisely matched. A controlled experiment at identical token budgets would cleanly isolate the effect of turn-level credit assignment.
