...
\midrule
Distributive & \textbf{0.723} & 0.633 & 0.680 & 0.676 \\
Compatible & 0.726 & 0.793 & \textbf{0.802} & 0.735 \\
Integrative & 0.776 & 0.715 & \textbf{0.920} & 0.872 \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Fair-only excels on compatible and integrative games.} Table~\ref{tab:lagrpo_arch_nash} shows agreed-only Nash ratios by archetype. Fair-only achieves the highest Nash ratios on compatible ($0.802$) and integrative games ($0.920$), with a particularly striking advantage on integrative ($0.920$ vs.\ $0.715$ for self-only and $0.776$ for base). On distributive games, however, the base model retains the highest Nash ratio ($0.723$); the LA-GRPO models are slightly more aggressive here, trading some balance for higher individual payoff (see following paragraph). Overall, the results indicate that the Nash product reward is particularly effective on integrative games when training is run to convergence.
