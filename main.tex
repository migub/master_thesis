...
\begin{enumerate}[label={}, leftmargin=0pt, itemsep=1.2em]
    ...
    \par This question tests whether applying LA-GRPO at the single-turn level improves learning stability and convergence in cooperative negotiations. Performance will be compared against baseline GRPO by measuring mean negotiation reward, convergence rate, and token efficiency across multiple training runs.
    ...
    \par This question investigates how different scalar reward compositions affect negotiation outcomes. Various weightings of self-utility, social welfare, and fairness ($\lambda$ parameters) will be evaluated through ablation studies. Success is indicated by higher joint welfare and Pareto efficiency without sacrificing individual payoff.
    ...
    \par This question examines which evaluation metrics best capture cooperative behavior. In addition to individual utility, outcomes will be assessed using social welfare, Nash product, and fairness indices. Pareto front analysis and equality metrics will be used to identify efficiency--fairness trade-offs.
    ...
    \par This question assesses whether cooperative agents remain effective when facing non-cooperative or adversarial counterparts. Experiments will pair trained models with selfish baselines and random agents to measure negotiation success rates and payoff degradation under defection.
    ...
    \par This question evaluates whether training for cooperation affects general reasoning and linguistic ability. The fine-tuned models will be tested on standard NLP benchmarks (MMLU-Pro, GLUE, IFEval, EQ-Bench) and compared to the base model to detect possible over-optimization or capability loss.
\end{enumerate}
...
Franceschetti formulated multi-turn negotiation as a Markov Decision Process (MDP), where states $s_t$ correspond to the dialogue history up to turn $t$, actions $a_t$ represent the next utterance, and a terminal reward $R(s_N)$ is assigned at the end of the negotiation based on the agreed outcome. Two agents alternate offers over $N=5$ rounds each, following an alternating-offer protocol.
...
\textcolor{gray}{[TODO: Add formal MDP definition following Franceschetti's notation. Include state transition diagram.]}

\subsection{Cooperative and Integrative Negotiation Theory}

\paragraph{Pareto Efficiency.} An outcome is Pareto-efficient if no alternative exists that makes one party better off without making the other worse off. In multi-issue negotiations, Pareto-efficient outcomes typically require agents to identify and exploit differences in issue importance---conceding on less-valued issues in exchange for gains on more-valued ones.

\paragraph{Nash Bargaining Solution.} \citet{Nash1950THEBP} introduced the Nash bargaining solution, defining fair and Pareto-efficient outcomes as the product of agents' utilities. The product $U_A \times U_B$ is the unique solution satisfying four axioms: Pareto efficiency, symmetry, scale invariance, and independence of irrelevant alternatives. This makes the Nash product a theoretically grounded single metric for cooperative success.

\paragraph{Unreasonable Fairness of Nash Welfare.} \citet{caragiannis2019unreasonable} demonstrated a remarkable result: maximizing Nash welfare (the product of utilities) simultaneously achieves envy-freeness up to one good \emph{and} Pareto efficiency in allocation problems. They call this ``unreasonable'' because Nash welfare was designed for efficiency, yet it also provides strong fairness guarantees. This provides the strongest theoretical argument for using the Nash product as the dominant cooperative term in the reward function of this work---it captures both efficiency and fairness in a single metric.

\paragraph{Integrative Bargaining.} Unlike single-issue distributive bargaining, multi-issue negotiations allow for log-rolling and package deals where parties trade concessions across issues with different importance weights \citep{lax1992manager_summary}. This is the mechanism through which cooperative agents can create value rather than merely distribute it.
...
\begin{tabular}{llll}
\toprule
\textbf{Method} & \textbf{Loss} & \textbf{Sampling} & \textbf{Pipeline} \\
\midrule
...
\paragraph{GRPO.} \citet{shao2024deepseekmath} introduced Group Relative Policy Optimization, an online reinforcement-learning method comparing grouped trajectories instead of using a value function. It generates $G$ independent trajectories per prompt and computes group-relative advantages $A_i = (R_i - \mu) / \sigma$. This on-policy method avoids the need for a value function but assigns the same advantage to every token in a trajectory, creating a noisy learning signal for multi-turn dialogues. GRPO is the precursor to LA-GRPO.
...
\paragraph{Prosocial Reward Shaping.} \citet{peysakhovich2017prosocial} trained deep RL agents with $R_i = (1 - \alpha) \times U_{\text{self}} + \alpha \times U_{\text{other}}$, showing that prosocial agents cooperate in social dilemmas even with novel partners.

\paragraph{Welfare Maximization.} \citet{pmlr-v162-liu22l} applied reinforcement-learning techniques to model fairness and efficiency trade-offs in multi-agent economic systems, connecting game-theoretic welfare optimization with machine learning.

\paragraph{Inequity Aversion.} \citet{hughes2018inequity} incorporated inequity aversion into multi-agent RL, demonstrating that agents can learn to avoid outcomes perceived as unfair.

\paragraph{Opponent Diversity.} \citet{mckee2020social} showed that diversity of training partners matters more than specific opponent characteristics for developing robust social behavior.

\paragraph{Linear Scalarization Limits.} \citet{hayes2022practical} established that linear scalarization can only discover solutions on the convex hull of the Pareto front. The reward formula used in this work partially circumvents this limitation because the Nash product term ($U_A \times U_B$) is inherently non-linear.

\paragraph{Strategic Reasoning in LLMs.} \citet{duan2024gtbenchuncoveringstrategicreasoning} evaluated LLMs' strategic reasoning abilities across diverse game-theoretic environments, highlighting limitations in long-term planning and strategic adaptation. \citet{jiang-akcakir-2025-explicit} empirically showed that explicit cooperation cues substantially improve negotiation success and human-likeness in LLM interactions. \citet{vaccaro2025advancingainegotiationsnew} reported large-scale evidence that agents using cooperative and empathetic language achieve higher joint gains than purely competitive agents.

\subsection{Summary and Research Gap}

LA-GRPO has been demonstrated only for competitive (zero-sum) negotiation. Cooperative reward shaping in multi-turn LLM dialogues remains unexplored. This thesis closes the gap by adapting LA-GRPO's turn-level credit assignment mechanism to a cooperative reward signal that balances self-interest, social welfare, and fairness.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=4.7in]{images/overview-approach.png}
...
\subsection{Cooperative Reward Design}
...
\subsubsection{Why Nash Product Should Dominate}

\citet{caragiannis2019unreasonable} showed that Nash welfare simultaneously delivers efficiency and fairness. The welfare term is largely redundant: it only adds unique signal in integrative/compatible games where total value varies but the split is already balanced. In distributive games ($\sim$31\% of the training set), welfare has zero variance and provides no learning signal whatsoever. Self-interest ($\lambda_{\text{self}}$) acts as a stabilizer---without sufficient self-interest weight, the agent can learn degenerate strategies (e.g., always conceding to ensure high Nash product).

While initial training metrics suggested that the all-equal configuration might underperform due to the welfare term's zero variance in distributive games, the full evaluation (Section~\ref{sec:results}) revealed a more nuanced picture: all-equal ultimately achieved the best overall performance, suggesting that the welfare signal contributes meaningfully in integrative settings despite being uninformative in distributive ones.


\subsection{Calculating Local Advantage}
...
\textcolor{gray}{[PLACEHOLDER: If LA-GRPO training runs were conducted with cooperative reward, present comparison to standard GRPO here. If only GRPO was used, explain that LA-GRPO was implemented but training was limited to GRPO due to compute constraints.]}
...
\subsection{Expected Effects and Hypotheses}
...
The LAMEN framework provides a rich negotiation environment with modular scenario definitions. Games, issues, agent roles, and negotiation protocol rules are specified through YAML configuration files, which are composed using the Hydra library for dynamic experiment setup.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=4.7in]{images/lamen-archetypes.png}
...
This distribution is a deterministic consequence of the issue pool composition: each issue has a fixed payoff structure (distributive or compatible), and the archetype of a two-issue game is fully determined by the types of its constituent issues. The original issues for the rental, loan, and merger scenarios are inherited from the LAMEN framework \citep{davidson2024evaluatinglanguagemodelagency}; the joint venture and employment contract issues were designed following the same principles. While alternative issue designs could have produced a more uniform archetype distribution, the issues were deliberately designed for \textit{realism} rather than balance: in an actual employment contract, salary and equity are inherently distributive (one party's gain is the other's cost), while training budget is naturally compatible (both employer and employee benefit from skill development). Artificially adding compatible issues to equalize the distribution would have sacrificed this ecological validity. Moreover, the resulting dominance of mixture games is desirable for this work, since these are precisely the mixed-motive settings that require agents to balance competitive and cooperative behavior---the core challenge this thesis addresses. Purely compatible games are trivially solved (both agents want the same outcome), and purely distributive games were already extensively studied by \citet{franceschetti2025advantage}. \citet{davidson2024evaluatinglanguagemodelagency} similarly found that cooperative bargaining games proved to be the most challenging for LLMs, further motivating their prominence in the training set.

Three game-type configurations are used during training and evaluation:
\begin{itemize}
    \item \texttt{multi-game}: All 5 scenarios (main experiment)
    \item \texttt{cooperative-only}: Joint Venture + Employment Contract only (ablation)
...
Two symmetric instances of the same model are assigned roles with distinct names, professions, ages, and personality types. Each agent has access only to its own private payoff table. The system prompt explicitly forbids revealing payoff information to the opponent. (Known issue: the model occasionally leaks payoff values early in training; this behavior diminishes as training progresses.)
...
Five $\lambda$ configurations are trained alongside a base-model baseline to isolate the effect of each reward component:

\begin{table}[H]
\centering
\caption{$\lambda$ configurations for the ablation study.}
\label{tab:lambda_configs}
\begin{tabular}{lccccc}
\toprule
\textbf{Run} & $\lambda_{\text{self}}$ & $\lambda_{\text{welfare}}$ & $\lambda_{\text{fair}}$ & \textbf{Checkpoint} & \textbf{Steps} \\
\midrule
Base Model & --- & --- & --- & none (no LoRA) & 0 \\
Self-Only & 1.0 & 0.0 & 0.0 & checkpoint-560 & 560 \\
Fair-Only & 0.0 & 0.0 & 1.0 & checkpoint-560 & 560 \\
Welfare-Only & 0.0 & 1.0 & 0.0 & checkpoint-598 & 598 \\
All-Equal & 1.0 & 1.0 & 1.0 & checkpoint-620 & 620 \\
Self-Fair-Equal & 0.5 & 0.0 & 0.5 & checkpoint-820 & 820 \\
\bottomrule
\end{tabular}
\end{table}
...
\subsubsection{Negotiation Outcome Metrics}

For each dialogue, the final agreements on all negotiated issues are parsed by the outcome extractor, and agent-specific rewards are computed as in Eq.~\ref{eq:rcoop_normalized}. From these values, additional aggregate metrics are derived.

The \textbf{joint welfare} captures total efficiency, i.e.\ how much total value the negotiation produced for both agents:
\begin{equation}
W = R_A + R_B.
\end{equation}

The \textbf{Nash product} measures fairness and balance between the agents, rewarding outcomes where both achieve high payoffs simultaneously:
\begin{equation}
N = R_A \times R_B.
\end{equation}

In addition, the following \textbf{normalized ratios} are computed, each in $[0, 1]$ and comparable across game types:
\begin{itemize}
    \item $\text{ratio\_self} = U_A / \max U_A$
    \item $\text{ratio\_welfare} = W / \max W$
    \item $\text{ratio\_nash} = N / \max N$
    \item $\text{ratio\_rcoop} = R_{\text{coop}} / \max R_{\text{coop}}$ (primary training metric)
\end{itemize}

\textbf{Agreed-only variants} of all metrics are computed only over successful negotiations, separating deal quality from agreement rate. A \textbf{per-archetype breakdown} further splits metrics by compatible, mixture, and integrative-distributive games.
...
\begin{itemize}
    ...
    \item 50 games per condition, temperature $= 1.0$, seed $= 42$.
\end{itemize}

\subsubsection{Robustness and Comparative Evaluation}

Each model---the base Qwen3-14B, GRPO baseline, and cooperative LA-GRPO variant---is evaluated against fixed, selfish, and random opponents to assess adaptability under mixed-motive conditions, following \citet{duan2024gtbenchuncoveringstrategicreasoning}. For each configuration, 100--200 episodes are sampled under identical random seeds. Mean reward, agreement rate, and tokens per reward point are reported with 95\% bootstrap confidence intervals.

\textcolor{gray}{[PLACEHOLDER: Robustness evaluation results not yet available.]}

\subsubsection{Language and Generalization Tests}

To verify that alignment on negotiation tasks does not harm general capabilities, the final models are re-evaluated on MMLU-Pro~\citep{wang2024mmluprorobustchallengingmultitask}, GLUE~\citep{wang2019gluemultitaskbenchmarkanalysis}, IFEval~\citep{zhou2023instructionfollowingevaluationlargelanguage}, EQ-Bench~\citep{paech2024eqbenchemotionalintelligencebenchmark}, and LM-Pragmatics~\citep{hu-etal-2023-fine}.

\textcolor{gray}{[PLACEHOLDER: Benchmark results not yet available. Required for RQ5.]}

\subsubsection{Success Criteria}

A model is considered successful if it achieves higher joint welfare and Nash product than GRPO, maintains comparable benchmark accuracy to the base model, and produces coherent, cooperative dialogues without degeneration.


\subsection{Ethical and Practical Considerations}

\textcolor{gray}{[PLACEHOLDER: Brief discussion of transparency, safety implications, and social alignment relevance of cooperative AI negotiators.]}
...
Figures~\ref{fig:training_performance} and~\ref{fig:training_quality} show the training dynamics across all five $\lambda$ configurations, logged via Weights \& Biases. All curves are smoothed with a centered running average (window $= 100$ data points).

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{images/training_curves_performance.pdf}
    \caption{Negotiation performance during training across five $\lambda$ configurations. All curves are smoothed with a centered running average (window $= 100$ data points). Shaded regions indicate variance. Top left: mean agent payoff $U_A$. Top right: agreement rate. Bottom left: normalized Nash ratio. Bottom right: normalized welfare ratio. Fair-only and all-equal show the strongest cooperative improvements, while self-only improves $U_A$ but not cooperative metrics. Welfare-only performs similarly to self-only on most metrics.}
    \label{fig:training_performance}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{images/training_curves_quality.pdf}
    \caption{Deal quality during training, computed only over successful negotiations. Left: agreed $U_A$ (excluding failed negotiations). Right: agreed Nash ratio. Self-only achieves the highest individual payoff on agreed deals, while fair-only leads on balanced outcomes. Welfare-only shows declining Nash ratio after step $\sim$400, suggesting over-optimization toward total value at the expense of balanced deals.}
    \label{fig:training_quality}
\end{figure}

\paragraph{Key findings.}

\textbf{Fair-only achieves higher agreement rate} (0.95+ vs.\ 0.78+). Note that both reward functions assign zero to failed negotiations ($U_A = 0$ implies $R_{\text{coop}} = 0$ regardless of $\lambda$ configuration). The difference lies in how they reward \emph{successful} negotiations: under self-only, the agent can obtain high reward from lopsided deals (e.g., $U_A = 80, U_B = 10$), but such aggressive offers are more likely to be rejected by the opponent. Under Nash product, only balanced outcomes yield high reward ($50 \times 50 = 2500 \gg 80 \times 10 = 800$), so the agent learns to propose offers the opponent is willing to accept. This is a central result for \textbf{RQ2}: the reward function directly shapes negotiation behavior and agreement rates.

\textbf{Self-only extracts more for itself} (higher ratio\_self), but at the cost of the opponent's payoff and overall efficiency. This confirms \textbf{H2}.

\textbf{Fair-only diverges more from the base model} (KL $0.22$--$0.62$ vs.\ $0.07$--$0.13$), indicating a stronger learning signal from the Nash product reward. While this is not yet in a concerning range, it warrants monitoring for potential instability. Reinforcement learning methods like PPO or GRPO are known to induce over-optimization or ``language drift'' when hyperparameters are mis-set \citep{pmlr-v235-jang24b, ouyang2022traininglanguagemodelsfollow}.

\textbf{All-equal appears weakest in early training metrics} (agreed/ratio\_self $= 0.423$, ratio\_nash $= 0.386$ at step $\sim$400). Based on these intermediate metrics, H3 hypothesized that the welfare term wastes gradient budget in distributive games. However, as Section~5.2 shows, the full evaluation after convergence tells a different story: all-equal ultimately becomes the best-performing model. This discrepancy highlights the danger of drawing conclusions from noisy mid-training metrics---the welfare signal may take longer to produce its effect but ultimately complements the other reward components.

\paragraph{Why self-only still achieves decent Nash product.} A purely selfish agent that never compromises receives reward $= 0$ (no agreement). To maximize $U_A$, the agent must reach deals, which requires offering the opponent acceptable terms. ``Good selfishness'' therefore naturally produces reasonable Nash products. The gap between self-only (ratio\_nash $\approx 0.58$) and fair-only (ratio\_nash $\approx 0.66$) is real but moderate, because both approaches converge toward a ceiling imposed by the frozen opponent.

\textcolor{gray}{[PLACEHOLDER: Per-archetype analysis --- how do models perform on compatible vs.\ distributive vs.\ integrative games?]}
...
Welfare-Only & 0/1/0 & \multicolumn{4}{c}{\textcolor{gray}{[PENDING: evaluation not yet conducted]}} \\
...
Self-Fair-Equal & 0.5/0/0.5 & 83.8\% & 57.0 \scriptsize{[54.4, 59.6]} & 0.757 \scriptsize{[0.727, 0.786]} & 0.625 \scriptsize{[0.596, 0.653]} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering
\caption{GRPO agreement rate (\%) by archetype across models.}
\label{tab:arch_agreement}
\renewcommand{\arraystretch}{1.15}
\begin{tabular}{lccccc}
\toprule
\textbf{Archetype} & \textbf{Base} & \textbf{Self-Only} & \textbf{Fair-Only} & \textbf{All-Equal} & \textbf{Self-Fair} \\
\midrule
single-compatible & 98.8 & 91.2 & 98.8 & 95.0 & \textbf{98.8} \\
single-distributive & 57.5 & 62.5 & \textbf{83.8} & \textbf{83.8} & 55.0 \\
single-integrative & 72.5 & 63.7 & 88.8 & \textbf{87.5} & 85.0 \\
non-int.\ compatible & 87.5 & 95.0 & 95.0 & 95.0 & \textbf{97.5} \\
non-int.\ distributive & 86.2 & 86.2 & \textbf{95.0} & \textbf{95.0} & 85.0 \\
integrative compatible & \textbf{100.0} & 90.0 & 91.2 & 97.5 & 92.5 \\
integrative distributive & 47.5 & 70.0 & 75.0 & \textbf{95.0} & 72.5 \\
\bottomrule
\end{tabular}
\end{table}
...
To evaluate whether turn-level credit assignment (\textbf{RQ1}) improves cooperative negotiation learning, three LA-GRPO models are trained with the same $\lambda$ configurations used for GRPO: self-only, fair-only, and all-equal. LA-GRPO trains for more steps (1340--2000) than GRPO (560--620) due to its more sample-efficient learning signal; each step updates the policy based on a single sampled turn $h$ rather than the full dialogue, requiring more iterations to cover the dialogue horizon. The same evaluation protocol (560 games, 20 repetitions, 7 archetypes) is applied.

\begin{table}[H]
\centering
\caption{LA-GRPO negotiation evaluation results (20 repetitions, 560 games per model).}
...
\begin{table}[H]
\centering
\caption{LA-GRPO agreed-only metrics (quality of successful negotiations).}
...
\subsection{GRPO vs.\ LA-GRPO Comparison (RQ1)}

Table~\ref{tab:grpo_vs_lagrpo} provides a direct comparison between GRPO and LA-GRPO for the three shared $\lambda$ configurations. Note that LA-GRPO was trained for more steps (1340--2000 vs.\ 560--620), as turn-level credit assignment requires more iterations to cover the full dialogue horizon. This means the comparison reflects the methods as practically deployed rather than at matched compute budgets.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{images/pareto_scatter_lagrpo.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{images/nash_product_comparison.pdf}
    \caption{Mean Nash product ($U_A \times U_B$) by model, computed over agreed negotiations only. LA-GRPO fair-only achieves the highest value (4775), indicating the most balanced and efficient deals. GRPO all-equal (4010) scores below the base model (4119) on this metric despite its superior aggregate performance---a consequence of closing many marginal deals that individually have lower quality.}
    \label{fig:nash_bar}
\end{figure}
...
The Nash product bar chart highlights that \textbf{LA-GRPO fair-only produces the highest-quality deals} of any model ($U_A \times U_B = 4775$), $16\%$ above the base model ($4119$). An interesting nuance is that GRPO all-equal scores \emph{below} the base model on this agreed-only metric ($4010$ vs.\ $4119$), despite being the strongest GRPO model on aggregate metrics (Table~\ref{tab:eval_results}). This apparent paradox is explained by GRPO all-equal's high agreement rate ($92.7\%$): it closes many marginal deals that individually have lower Nash product, but the sheer volume of deals compensates in aggregate metrics.


\subsection{Out-of-Domain Generalization: Trust Game}

To evaluate whether cooperative training generalizes beyond the negotiation domain, all trained model variants are tested on the Trust Game---a canonical game-theoretic setting the models were never trained on.

\subsubsection{vs\_base: Trained Model vs.\ Frozen Base Model}

\begin{table}[H]
\centering
\caption{Trust Game results: trained model as Investor vs.\ frozen base model as Trustee.}
\label{tab:trust_investor}
\begin{tabular}{lcccc}
\toprule
\textbf{Metric} & \textbf{Base} & \textbf{Self-Only} & \textbf{Fair-Only} & \textbf{All-Equal} \\
\midrule
Avg send & 5.00 ($\pm$0.00) & 5.38 ($\pm$0.49) & \textbf{5.48} ($\pm$0.74) & 5.04 ($\pm$0.20) \\
Investor payoff & 16.02 & 15.04 & 15.08 & 15.86 \\
Trustee payoff & 3.98 & 5.72 & 5.88 & 4.22 \\
Social welfare & 20.00 & 20.76 & \textbf{20.96} & 20.08 \\
Nash product & 62.5 & 80.8 & \textbf{83.1} & 65.1 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering
\caption{Trust Game results: frozen base model as Investor, trained model as Trustee.}
\label{tab:trust_trustee}
\begin{tabular}{lcccc}
\toprule
\textbf{Metric} & \textbf{Base} & \textbf{Self-Only} & \textbf{Fair-Only} & \textbf{All-Equal} \\
\midrule
Return ratio & 72.4\% & 74.4\% & \textbf{78.8\%} & \textcolor{gray}{[INSERT]} \\
Investor payoff & 15.86 & 16.16 & \textbf{16.82} & \textcolor{gray}{[INSERT]} \\
Trustee payoff & 4.14 & 3.84 & 3.18 & \textcolor{gray}{[INSERT]} \\
Nash product & 64.4 & 59.3 & 47.0 & \textcolor{gray}{[INSERT]} \\
\bottomrule
\end{tabular}
\end{table}

\subsubsection{vs\_self: Selfplay}
...
\textbf{Metric} & \textbf{Base} & \textbf{Self-Only} & \textbf{Fair-Only} & \textbf{All-Equal} \\
\midrule
Avg send & 5.00 ($\pm$0.00) & 5.38 ($\pm$0.49) & 5.46 ($\pm$0.73) & 5.04 ($\pm$0.20) \\
Social welfare & 20.00 & 20.76 & \textbf{20.92} & 20.08 \\
...
\paragraph{Cooperative training transfers modestly to the Trust Game.} Fair-only as investor sends more ($5.48$ vs.\ $5.00$ baseline), breaking the base model's zero-variance ``always send half'' pattern. The resulting Nash product improvement ($83.1$ vs.\ $62.5$) represents a 33\% increase. The effect is small in absolute terms but points in the right direction: the model learned a cooperative disposition that transfers to an unseen game.

\paragraph{Fair-only learns generosity, not strategic cooperation.} As trustee, the fair-only model returns 78.8\% of received points (vs.\ 72.4\% baseline), making the investor better off but itself worse off (payoff $3.18$ vs.\ $3.98$). In negotiation, generosity is reciprocated; in the Trust Game's trustee role, it is one-directional and exploitable. The model learned cooperative behavior that is context-appropriate in negotiation but suboptimal in the Trust Game's asymmetric structure.

\paragraph{All-equal shows minimal transfer to the Trust Game,} despite being the strongest model in the negotiation evaluation. This suggests that the Trust Game's simple two-turn structure does not benefit from the multi-objective reward signal that makes all-equal effective in complex multi-issue negotiations.
...
\paragraph{Selfplay reveals a nuance.} In selfplay, self-only achieves slightly higher Nash product ($76.2$) than fair-only ($74.9$). When the trustee's generosity benefits itself (same model playing both roles), the dynamic changes: fair-only's tendency to over-return does not lead to exploitation.


\subsection{Robustness to Defection (RQ4)}

\textcolor{gray}{[PLACEHOLDER: Tests against adversarial or selfish opponents have not yet been conducted. Planned approaches:}
\begin{itemize}
    \item \textcolor{gray}{Cooperative model vs.\ self-only model (cross-evaluation)}
    \item \textcolor{gray}{Cooperative model vs.\ GPT-4o-mini as opponent}
    \item \textcolor{gray}{Analysis of whether the cooperative agent can detect and respond to exploitation}
\end{itemize}
\textcolor{gray}{]}
...
\textcolor{gray}{[PLACEHOLDER: MMLU-Pro, GLUE, IFEval, EQ-Bench results have not yet been generated.}

\textcolor{gray}{Expected outcome: Minimal capability loss, since LoRA trains only $\sim$0.26\% of parameters and KL regularization constrains policy drift. However, the elevated KL divergence in the fair-only run (0.22--0.62) warrants careful checking. Degeneration may manifest as repetitive phrasing, typographical artifacts, or overly literal negotiation styles---behaviors documented in earlier GRPO experiments \citep{franceschetti2025advantage}.]}


\subsection{Comparative Discussion}

\paragraph{RQ1 (LA-GRPO).} Turn-level credit assignment improves cooperative negotiation learning, but the effect is reward-dependent. For single-objective rewards, LA-GRPO provides substantial gains: self-only achieves $95.5\%$ agreement (vs.\ $79.8\%$ GRPO), and fair-only achieves the highest per-deal Nash ratio of any model ($0.787$ agreed). For the blended all-equal reward, however, LA-GRPO and GRPO perform nearly identically, suggesting that multi-objective rewards already provide sufficient gradient diversity without turn-level decomposition. The combination of LA-GRPO with Nash product reward ($\lambda_{\text{fair}} = 1.0$) emerges as the most effective configuration for cooperative negotiation quality.
...
\paragraph{RQ4 (Robustness).} \textcolor{gray}{[PLACEHOLDER: Awaiting robustness evaluation results.]}

\paragraph{RQ5 (Capabilities).} \textcolor{gray}{[PLACEHOLDER: Awaiting benchmark results.]}
...
\begin{enumerate}
    ...
    \item \textbf{$\lambda$ ablation study} across five reward configurations demonstrating that combining all three objectives ($\lambda_{\text{self}} = \lambda_{\text{welfare}} = \lambda_{\text{fair}} = 1.0$) yields the best overall GRPO performance: $+21\%$ on $U_A$, $+14$ percentage points on agreement rate, and $+18\%$ on Nash product over the base model. The Nash product is the strongest individual cooperative signal (fair-only $\gg$ self-only), but dropping the welfare component significantly hurts performance, refuting the initial hypothesis that welfare would be redundant.

    \item \textbf{LA-GRPO with cooperative reward} demonstrates that turn-level credit assignment and fairness-oriented rewards are complementary: LA-GRPO fair-only achieves the highest per-deal Nash ratio ($0.787$ agreed) of any model, surpassing all GRPO configurations. For single-objective rewards, LA-GRPO substantially outperforms GRPO; for blended rewards, the methods perform comparably.
...
\begin{itemize}
    \item \textbf{Single GPU, no vLLM:} Sequential dialogue generation ($\sim$80 forward passes per training step) limits the scale of training. A vLLM colocate implementation plan exists but was not realized.
    ...
    \item \textbf{Unmatched step counts between GRPO and LA-GRPO:} LA-GRPO was trained for 1340--2000 steps compared to 560--620 for GRPO. While this reflects the practical reality that turn-level credit assignment requires more iterations, it prevents a perfectly controlled comparison at matched compute budgets.
    ...
    \item \textbf{Partial agreement bug:} The \texttt{get\_payoffs()} function returns positive rewards for partial agreements in multi-issue games, slightly distorting the training signal.

    \item \textcolor{gray}{[PLACEHOLDER: Benchmark results missing --- RQ5 not fully answered.]}
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Matched-step GRPO vs.\ LA-GRPO comparison:} The current comparison uses different step counts (560--620 vs.\ 1340--2000). A controlled experiment at matched compute budgets would isolate the effect of turn-level credit assignment from additional training time.

    \item \textbf{Iterated Trust Game:} Multiple rounds with reputation and reciprocity would test whether cooperative training produces robust tit-for-tat-like strategies.

    \item \textbf{vLLM colocate mode:} Batched generation for $>$10$\times$ training speedup, enabling larger-scale experiments and more $\lambda$ configurations.
...
Cooperative LLM negotiators could be deployed in real-world applications ranging from automated contract negotiation to diplomatic support systems. The balance between cooperation and robustness against exploitation mirrors fundamental challenges in AI alignment: models must be helpful and fair while remaining resistant to manipulation. This thesis provides initial evidence that reinforcement learning with appropriately designed cooperative rewards can shift LLM behavior toward mutual benefit---a small but meaningful step toward AI systems that create value for all parties rather than extracting it from opponents.
...
\label{app:prompt}
\textcolor{gray}{[PLACEHOLDER: System prompt from \texttt{env.py} showing how scenarios, roles, and payoff tables are formatted for the model.]}

\section{Game Configuration Examples}
\label{app:game_configs}
\textcolor{gray}{[PLACEHOLDER: Example YAML files for Joint Venture and Employment Contract scenarios, showing issue definitions and payoff tables.]}

\section{Example Mixed-Motive Self-Play Dialogue}
\label{app:example_dialogue}

\begin{figure}[H]
    \centering
    \includegraphics[width=4.7in]{images/mixed-motive-example.png}
...
\section{Trust Game Prompt and Full Results}
\label{app:trust_game}
\textcolor{gray}{[PLACEHOLDER: Full prompt used in \texttt{trust\_game\_eval.py} for both investor and trustee roles. Complete tables for all vs\_base and vs\_self conditions with all metrics including standard deviations.]}

\section{WandB Training Curves}
\label{app:wandb}
\textcolor{gray}{[PLACEHOLDER: Exported learning curves showing ratio\_rcoop, agreement rate, KL divergence, and per-archetype metrics over training steps for all three runs.]}
