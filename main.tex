...
Large Language Models (LLMs) are increasingly used to negotiate, advise, and act on behalf of people. When cooperative or fair behaviour is desired, it is typically requested through system prompts---instructions like ``be fair'' or ``find a mutually beneficial deal''. This thesis investigates whether cooperative negotiation behaviour can instead be trained directly into an LLM, so that fairness and joint value creation become part of the model's default behaviour rather than a prompt-level instruction that can be ignored, overridden, or forgotten.

The work builds on \citet{franceschetti2025advantage}, who introduced Local Advantage Group Relative Policy Optimization (LA-GRPO) for competitive (zero-sum) negotiation. This thesis extends LA-GRPO from competitive to cooperative settings by replacing the original self-interest reward with a scalar reward that combines three components: the agent's own payoff, the social welfare ($U_A + U_B$), and the Nash product ($U_A \times U_B$). To make this signal usable across heterogeneous negotiation games, each component is normalized by the per-game maximum---so that distributive, integrative, and compatible games contribute equally to the gradient. Five reward configurations are trained with GRPO, three of them additionally with LA-GRPO. The base model is Qwen3-14B-Instruct, fine-tuned with LoRA adapters on a single rented GPU.

The trained models are evaluated on 560 negotiations each from an extended LAMEN environment (5 scenarios, 19 issues, 47 game configurations), on a held-out out-of-sample game set, on two out-of-domain transfer tests (the Trust Game and GT-HarmBench, an AI-safety benchmark covering seven game-theoretic structures), against four adversarial opponent personas (hardball, deceptive, anchoring, stubborn), and on three standard capability benchmarks (MMLU-Pro, IFEval, GSM8K).

The main results are:

\begin{itemize}
    \item \textbf{Cooperative reward design works.} The all-equal reward is the strongest GRPO configuration, raising agreement rate from $78.6\%$ to $92.7\%$ and nearly doubling agreement on the hardest archetype ($47.5\% \to 95.0\%$).
    \item \textbf{Fairness is the strongest single signal.} The Nash product reward clearly outperforms self-interest alone, which improves the agent's own payoff but not cooperation.
    \item \textbf{LA-GRPO helps, but is nuanced.} At convergence, LA-GRPO outperforms GRPO on single-objective rewards (fair-only achieves the highest per-deal Nash ratio). At matched compute, GRPO is faster on single-objective rewards while LA-GRPO retains a slight edge only on the blended reward.
    \item \textbf{Cooperative training transfers and stays robust.} General capabilities are preserved ($\pm 0.6$ pp on MMLU-Pro, IFEval, GSM8K), behaviour transfers modestly to two out-of-domain settings (the Trust Game and GT-HarmBench), and the model substantially outperforms the base against three of four adversarial personas---cooperative training does not produce an exploitable agent.
\end{itemize}

The broader contribution is a small step toward what might be called \emph{intuitive} moral alignment: cooperative dispositions encoded in the model's weights through reward shaping, rather than instructed at inference time through prompts. This matters as LLMs are increasingly deployed in autonomous, agentic settings where prompt-level guardrails become less reliable.

\newpage
\section*{Acknowledgements}
\addcontentsline{toc}{section}{Acknowledgements}

I would like to thank my supervisors Oliver Staubli (HSLU) and Peter Niederberger (Tincan AG) for supporting this thesis and providing feedback throughout the process. I am also grateful to Luca Franceschetti, whose preceding work on LA-GRPO laid the foundation for this thesis and whose code and documentation made the methodological extension possible.
...
LA-GRPO   & Local Advantage Group Relative Policy Optimization \\
LAMEN     & Language Agent for Multi-Issue Negotiation (framework) \\
LLM       & Large Language Model \\
LoRA      & Low-Rank Adaptation \\
MARL      & Multi-Agent Reinforcement Learning \\
MDP       & Markov Decision Process \\
MMLU-Pro  & Massive Multitask Language Understanding -- Professional (benchmark) \\
nf4       & 4-bit Normal Float (quantization format) \\
PPO       & Proximal Policy Optimization \\
REFUEL    & Refining LLMs with Offline Multi-Turn Reinforcement Learning \\
...
\textbf{Resource} & \textbf{Source} \\
\hline
Qwen3-14B-Instruct (base model) & \texttt{OpenPipe/Qwen3-14B-Instruct} (HuggingFace) \\
LAMEN framework & \citet{davidson2024evaluatinglanguagemodelagency} \\
LA-GRPO reference implementation & \citet{franceschetti2025advantage} \\
Training and evaluation code & \url{github.com/migub/multiturn-llm-training} \\
GRPO training experiment logs (W\&B Report) & \url{https://api.wandb.ai/links/michael-gubler-hochschule-luzern/ig05od0g} \\
LA-GRPO training experiment logs (W\&B Report) & \url{https://api.wandb.ai/links/michael-gubler-hochschule-luzern/dlwxgofj} \\
LM Evaluation Harness & \citet{eval-harness} \\
MMLU-Pro benchmark dataset & \texttt{TIGER-Lab/MMLU-Pro} (HuggingFace) \\
IFEval benchmark dataset & \texttt{google/IFEval} (HuggingFace) \\
GSM8K benchmark dataset & \texttt{openai/gsm8k} (HuggingFace) \\
GT-HarmBench benchmark & \citet{cobben2026gtharmbench}; results at \texttt{Agentic-Moral-Alignment/gt-harmbench-eval} (HuggingFace) \\
\end{tabularx}
...
\begin{table}[H]
\centering
\caption[$\lambda$ configurations and training methods.]{$\lambda$ configurations and training methods. LA-GRPO trains for more steps but uses fewer tokens per step (shared dialogue prefix generated once per group).}
...
\subsection{Training Dynamics and Convergence}\label{sec:training_dynamics}
...
\begin{figure}[H]
    ...
    \caption[GRPO negotiation performance during training.]{GRPO negotiation performance during training across five $\lambda$ configurations. All curves are smoothed with a centered running average (window $= 100$ data points). Shaded regions indicate variance. Top left: mean agent payoff $U_A$. Top right: agreement rate. Bottom left: normalized Nash ratio. Bottom right: normalized welfare ratio.}
...
\begin{figure}[H]
    ...
    \caption[GRPO deal quality during training.]{GRPO deal quality during training, computed only over successful negotiations. Left: agreed $U_A$ (excluding failed negotiations). Right: agreed Nash ratio.}
    \label{fig:training_quality}
\end{figure}

\paragraph{Overview of GRPO training.} Across the five configurations, fair-only and all-equal show the strongest improvements on cooperative metrics (agreement rate, Nash ratio, welfare ratio), while self-only improves $U_A$ but not cooperative outcomes. Welfare-only performs similarly to self-only on most metrics. Looking at deal quality (Figure~\ref{fig:training_quality}), self-only achieves the highest individual payoff on agreed deals, while fair-only produces the most balanced outcomes. Welfare-only shows a declining Nash ratio after step $\sim$400, suggesting over-optimization toward total value at the expense of balanced deals.
...
The three LA-GRPO configurations (self-only, fair-only, all-equal) were trained for up to 2000 steps.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{images/lagrpo_training_curves_performance.pdf}
    \caption[LA-GRPO negotiation performance during training.]{LA-GRPO negotiation performance during training across three $\lambda$ configurations (up to 2000 steps). Same smoothing and layout as Figure~\ref{fig:training_performance}.}
...
\begin{figure}[H]
    ...
    \caption[LA-GRPO deal quality during training.]{LA-GRPO deal quality during training, computed only over successful negotiations. Left: agreed $U_A$. Right: agreed Nash ratio.}
    \label{fig:lagrpo_training_quality}
\end{figure}

\paragraph{Overview of LA-GRPO training.} Self-only's agreed $U_A$ rises sharply, indicating increasingly aggressive value extraction, while its agreed Nash ratio remains flat. Fair-only reaches the highest agreed Nash ratio ($\sim 0.70$) around step 1000--1400 before slowly declining---a pattern that influenced checkpoint selection (discussed below).

\paragraph{Self-only continues to improve.} Unlike GRPO self-only, which plateaus, LA-GRPO self-only shows continued improvement in $U_A$ over the full 2000 training steps. This suggests that turn-level credit assignment extracts more learning signal from the same reward.
...
\begin{table}[H]
\centering
\caption[GRPO negotiation evaluation results.]{GRPO negotiation evaluation results (20 repetitions, 560 games per model). Bold indicates best per column. 95\% bootstrap confidence intervals in brackets.}
...
\paragraph{Fairness drives cooperation, self-interest does not.} Self-only ($\lambda_{\text{self}} = 1.0$) improves $U_A$ marginally but does \emph{not} improve agreement rate, ratio Nash, or ratio welfare significantly over the baseline. Fair-only, in contrast, significantly outperforms self-only on agreement rate ($p < 0.001$), ratio Nash ($p < 0.001$), and ratio welfare ($p < 0.01$). The cooperative reward signals (welfare, fairness) are what drive meaningful improvement in negotiation quality.
...
\begin{table}[H]
\centering
\caption[GRPO agreement rate (\%) by archetype across models.]{GRPO agreement rate (\%) by archetype across models. Bold indicates best per row.}
...
To evaluate whether turn-level credit assignment (\textbf{RQ1}) improves cooperative negotiation learning, three LA-GRPO models are trained with the same $\lambda$ configurations used for GRPO: self-only, fair-only, and all-equal. The same evaluation protocol (560 games, 20 repetitions, 7 archetypes) is applied.

\begin{table}[H]
\centering
\caption[LA-GRPO negotiation evaluation results.]{LA-GRPO negotiation evaluation results (20 repetitions, 560 games per model). Bold indicates best per column.}
...
\midrule
Base Model & --- & 0 & 78.6\% & 49.9 & 0.702 & 0.585 \\
...
\begin{table}[H]
\centering
\caption[LA-GRPO agreed-only metrics (quality of successful negotiations).]{LA-GRPO agreed-only metrics (quality of successful negotiations). Bold indicates best per column.}
...
\begin{table}[H]
\centering
\caption[LA-GRPO agreed-only Nash ratio by archetype.]{LA-GRPO agreed-only Nash ratio by archetype. Bold indicates best per row. The LA-GRPO evaluation uses a reduced game suite covering three core archetypes (200 games total per model).}
\label{tab:lagrpo_arch_nash}
\renewcommand{\arraystretch}{1.15}
\begin{tabular}{lcccc}
\toprule
\textbf{Archetype} & \textbf{Base} & \textbf{Self-Only} & \textbf{Fair-Only} & \textbf{All-Equal} \\
\midrule
single-compatible & 0.670 & 0.793 & \textbf{0.802} & 0.735 \\
single-distributive & \textbf{0.723} & 0.633 & 0.680 & 0.676 \\
single-integrative & 0.864 & 0.715 & \textbf{0.920} & 0.872 \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Fair-only excels on compatible and integrative games.} Table~\ref{tab:lagrpo_arch_nash} shows agreed-only Nash ratios by archetype. Fair-only achieves the highest Nash ratios on single-compatible ($0.802$) and single-integrative games ($0.920$), with a particularly striking advantage on single-integrative ($0.920$ vs.\ $0.715$ for self-only). On single-distributive games, however, the base model retains the highest Nash ratio ($0.723$); the LA-GRPO models trade some balance for higher individual payoff here (see following paragraph). Overall, the results indicate that the Nash product reward is particularly effective on integrative games when training is run to convergence.

\paragraph{Self-only negotiates aggressively.} The mean payoffs $(U_A / U_B)$ confirm that self-only splits outcomes in its own favor: $56.0/44.0$ on distributive games and $65.8/46.1$ on integrative games. Fair-only produces more symmetric payoffs ($54.4/45.6$ on distributive and $55.9/60.5$ on integrative), and all-equal even concedes a slight advantage to the opponent on distributive games ($47.6/52.4$).


\subsection{GRPO vs.\ LA-GRPO Comparison (RQ1)}\label{sec:grpo_vs_lagrpo}

Table~\ref{tab:grpo_vs_lagrpo} provides a direct comparison between GRPO and LA-GRPO for the three shared $\lambda$ configurations at their respective convergence points. LA-GRPO was trained for more steps (1340--2000 vs.\ 560--620).

\begin{table}[H]
\centering
\caption[GRPO vs.\ LA-GRPO comparison on the same $\lambda$ configurations.]{GRPO vs.\ LA-GRPO comparison on the same $\lambda$ configurations. Agreed ratio Nash measures per-deal cooperative quality.}
...
\midrule
\multirow{2}{*}{Self-Only (1/0/0)} & GRPO & 560 & 79.8\% & 54.9 & 0.581 & \textbf{0.728} \\
...
\addlinespace
\multirow{2}{*}{All-Equal (1/1/1)} & GRPO & 620 & \textbf{92.7\%} & \textbf{60.3} & \textbf{0.692} & \textbf{0.747} \\
...
\paragraph{LA-GRPO substantially improves self-only at convergence.} The largest effect occurs for self-only: LA-GRPO achieves $95.5\%$ agreement (vs.\ $79.8\%$ for GRPO), $U_A = 68.4$ (vs.\ $54.9$), and ratio Nash $= 0.684$ (vs.\ $0.581$). Turn-level credit assignment allows the self-interested agent to learn which specific utterances lead to deal closure, rather than attributing the entire dialogue's outcome uniformly across all tokens.

\paragraph{LA-GRPO fair-only achieves the highest per-deal cooperation.} With an agreed Nash ratio of $0.787$, LA-GRPO fair-only produces the most balanced and efficient deals of any model in this study---higher than any GRPO configuration (best: $0.747$).

\paragraph{All-equal shows no clear benefit from LA-GRPO at convergence.} GRPO and LA-GRPO perform nearly identically for the blended reward ($92.7\%$ vs.\ $92.0\%$ agreement, $0.692$ vs.\ $0.680$ ratio Nash). A possible explanation is that the multi-objective reward signal is already sufficiently informative at the dialogue level, and turn-level decomposition does not add meaningful additional credit assignment for this reward configuration.

\subsection{Pareto and Fairness Metrics}\label{sec:pareto_fairness}
...
\begin{figure}[H]
    ...
    \caption[$U_A$ vs.\ $U_B$ for all GRPO models.]{$U_A$ vs.\ $U_B$ for all GRPO models (agreed negotiations only). Diamond marks the mean outcome. Red crosses at origin indicate failed negotiations.}
...
\begin{figure}[H]
    ...
    \caption[$U_A$ vs.\ $U_B$ for all LA-GRPO models.]{$U_A$ vs.\ $U_B$ for all LA-GRPO models (agreed negotiations only).}
    \label{fig:pareto_lagrpo}
\end{figure}

\paragraph{Negotiation styles differ by reward function.} The scatter plots reveal distinct negotiation \emph{styles} shaped by the reward function. Self-only models (both GRPO and LA-GRPO) produce outcomes skewed below the diagonal---the agent extracts more value at the opponent's expense. Fair-only models cluster closer to the diagonal, reflecting the Nash product's preference for balanced outcomes; LA-GRPO fair-only in particular clusters most tightly around the diagonal. All-equal falls between the two extremes.

Figure~\ref{fig:nash_bar} compares the agreed-only game-normalized Nash ratio across configurations. This metric captures per-deal cooperative quality, isolated from agreement rate effects: a high agreed Nash ratio means that when a deal is closed, it tends to be both efficient (high total value) and balanced (fair split).

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{images/nash_product_comparison.pdf}
    \caption[Agreed game-normalized Nash ratio by $\lambda$ configuration.]{Agreed game-normalized Nash ratio by $\lambda$ configuration, restricted to successful negotiations. The Nash ratio is the achieved Nash product divided by the maximum possible Nash product for that game (in $[0, 1]$). Bars are grouped by reward configuration to enable direct GRPO vs.\ LA-GRPO comparison; the dashed line shows the base model reference ($0.744$, derived from overall Nash $0.585$ and agreement rate $78.6\%$). The y-axis starts at $0.5$ to emphasize differences.}
    \label{fig:nash_bar}
\end{figure}

\paragraph{Cooperative training mainly improves agreement, not deal quality.} The agreed Nash ratio bar chart adds context to the deal quality findings reported above. Compared to the base model reference of $0.744$, only LA-GRPO fair-only ($0.787$) substantially improves per-deal cooperation. Self-only configurations fall slightly below the base model, suggesting that the higher agreement rates of trained self-only models come at a small cost to individual deal quality. All-equal configurations cluster near the base model level. This indicates that cooperative training primarily improves the agreement rate---the willingness to close deals---rather than the quality of those deals. Only the Nash-product reward, particularly when combined with LA-GRPO's turn-level credit assignment, also improves what each deal actually achieves.


\subsection{Out-of-Domain Generalization}

To evaluate whether cooperative training generalizes beyond the negotiation domain, all trained model variants are tested on two unseen settings. The first is the Trust Game, a canonical behavioral-economics setting that probes trust and reciprocity through continuous send/return decisions. The second is GT-HarmBench \citep{cobben2026gtharmbench}, a recent benchmark that evaluates discrete action choices across 2{,}009 high-stakes AI-safety scenarios mapped to seven game-theoretic structures. Together, these two evaluations probe transfer to settings of very different scale and structure.

\subsubsection{Trust Game}

The Trust Game is a canonical game-theoretic setting the models were never trained on. GRPO models were evaluated over 50 games per condition; LA-GRPO models were evaluated over 300 games per condition. Two conditions are tested: each trained model plays against the frozen base model (Tables~\ref{tab:trust_investor} and~\ref{tab:trust_trustee}), and each trained model plays against itself in selfplay (Table~\ref{tab:trust_selfplay}).
...
\begin{table}[H]
\centering
\caption[Trust Game results: trained model as Investor.]{Trust Game results: trained model as Investor vs.\ frozen base model as Trustee. Bold indicates best per row.}
...
\begin{table}[H]
\centering
\caption[Trust Game results: trained model as Trustee.]{Trust Game results: frozen base model as Investor, trained model as Trustee. Bold indicates best per row (excluding Investor payoff, where higher means the trustee returned more). The base sends exactly 5 with zero variance.}
...
Nash product & 60.9 & 59.3 & 47.0 & 58.7 & 57.0 & 61.3 & \textbf{65.6} \\
\bottomrule
\end{tabular}%
}
\end{table}
...
\paragraph{Selfplay reveals a different picture.} When the same trained model plays both investor and trustee roles, GRPO self-only achieves the highest Nash product ($76.2$), followed closely by GRPO fair-only ($74.9$). Among LA-GRPO models, the differences are smaller (Nash $64.6$--$68.4$). In selfplay, the trustee's generosity benefits the model itself, so fair-only's tendency to return more no longer leads to exploitation.

\paragraph{H4 is partially supported.} The Trust Game results indicate that hypothesis H4 (Section~\ref{sec:hypotheses}), which predicted that cooperative training generalizes to unseen games, is only partially supported. GRPO fair-only clearly transfers a cooperative disposition to the Trust Game (sending more as investor, achieving higher Nash product); LA-GRPO models, however, show weaker generalization---their investor behavior remains close to the base model's baseline of $5.00$. The GT-HarmBench evaluation below provides additional evidence in support of the same pattern: rewards with an opponent-aware component (fair-only, all-equal) generalize positively, while pure self-interest generalizes negatively.

\subsubsection{AI Safety Scenarios (GT-HarmBench)}

To probe transfer at a larger scale, all trained model variants are also evaluated on GT-HarmBench \citep{cobben2026gtharmbench}, a recent benchmark of $2{,}009$ high-stakes scenarios drawn from the MIT AI Risk Repository and mapped to seven game-theoretic structures (Prisoner's Dilemma, Stag Hunt, Chicken, Bach or Stravinski, Coordination, Matching Pennies, No Conflict). In each scenario, the model selects one of two actions; the key metrics are the \textit{utilitarian rate} (fraction of valid actions that maximize the sum of payoffs---the socially beneficial choice) and the \textit{Nash rate} (fraction matching the egoistic Nash equilibrium action, which in Prisoner's Dilemma corresponds to defection). Evaluations follow the benchmark's default \texttt{bare} environment with \texttt{before}-style reasoning, free-form generation, and a single fixed seed, yielding $2{,}265$ classifiable decisions per model (results vary slightly from $2{,}009$ scenarios because some scenarios contribute multiple decision points).

\begin{table}[H]
\centering
\caption[GT-HarmBench results across all formal games.]{GT-HarmBench results across all formal games ($n = 2{,}265$ per model). Higher utilitarian rate indicates more socially beneficial behavior. Bold indicates best per column.}
\label{tab:gtharmbench_overall}
\renewcommand{\arraystretch}{1.15}
\begin{tabular}{lccc}
\toprule
\textbf{Model} & \textbf{Valid} & \textbf{Nash (defect)} & \textbf{Utilitarian} \\
\midrule
Base Model & 0.992 & 0.639 & 0.917 \\
GRPO self-only & \textbf{0.993} & 0.639 & 0.916 \\
GRPO fair-only & \textbf{0.993} & \textbf{0.631} & \textbf{0.935} \\
GRPO welfare-only & 0.992 & 0.640 & 0.918 \\
GRPO all-equal & 0.990 & 0.636 & 0.919 \\
GRPO self-fair-equal & 0.992 & 0.643 & 0.913 \\
LA-GRPO self-only & 0.990 & 0.646 & 0.909 \\
LA-GRPO fair-only & 0.991 & 0.638 & 0.915 \\
LA-GRPO all-equal & 0.991 & 0.633 & 0.927 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering
\caption[GT-HarmBench results: Prisoner's Dilemma only.]{GT-HarmBench results restricted to Prisoner's Dilemma scenarios ($n = 654$ per model). PD is the canonical setting where reward-function differences should be most visible. Bold indicates best per column.}
\label{tab:gtharmbench_pd}
\renewcommand{\arraystretch}{1.15}
\begin{tabular}{lcc}
\toprule
\textbf{Model} & \textbf{Nash (defect)} & \textbf{Utilitarian (cooperate)} \\
\midrule
Base Model & 0.144 & 0.852 \\
GRPO self-only & 0.144 & 0.852 \\
GRPO fair-only & \textbf{0.115} & \textbf{0.881} \\
GRPO welfare-only & 0.147 & 0.847 \\
GRPO all-equal & 0.135 & 0.859 \\
GRPO self-fair-equal & 0.157 & 0.838 \\
LA-GRPO self-only & 0.167 & 0.826 \\
LA-GRPO fair-only & 0.142 & 0.855 \\
LA-GRPO all-equal & 0.127 & 0.872 \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Fair-only with GRPO produces the most pro-social behavior.} Across all $2{,}265$ scenarios, GRPO fair-only achieves the highest utilitarian rate ($0.935$, $+1.8$ percentage points over the base model). The effect is most pronounced in Prisoner's Dilemma, where GRPO fair-only reaches a cooperation rate of $0.881$ ($+2.9$ pp) and reduces Nash-defection by $2.9$ pp. This is consistent with the negotiation findings (Section~\ref{sec:pareto_fairness}) and the Trust Game findings: fair-only's Nash-product reward, which explicitly couples agent and opponent utility, transfers a pro-social disposition that is detectable even in single-shot, discrete-action settings far removed from the training distribution.

\paragraph{LA-GRPO all-equal also generalizes positively.} Among LA-GRPO variants, the all-equal configuration shows the strongest transfer (overall utilitarian $0.927$, PD utilitarian $0.872$). This is interesting because LA-GRPO all-equal was \textit{not} the strongest in-domain negotiator, suggesting that the broader multi-objective reward provides a more transferable inductive bias for unseen game-theoretic situations than the per-deal Nash-product gains observed in negotiation.

\paragraph{Self-only training degrades pro-social behavior.} LA-GRPO self-only is the only configuration that performs \textit{worse} than the base model on the utilitarian metric ($0.909$ overall, $0.826$ in PD; $-2.6$ pp). It also has the highest Nash rate ($0.646$). This mirrors the in-domain finding (Section~\ref{sec:training_dynamics}) that pure self-interest learns aggressive bargaining and shows that the effect generalizes to discrete-action AI-safety scenarios as well.

\paragraph{Effect sizes are small but consistent.} The overall changes vs.\ base are modest---between $-2.6$ and $+1.8$ percentage points---because all evaluated configurations stem from a relatively cooperative base model that already chooses the utilitarian action $91.7\%$ of the time on this benchmark. The signal nonetheless aligns with the negotiation and Trust Game results: rewards that include an opponent-aware component (fair-only, all-equal) generalize positively, while pure self-interest generalizes negatively. The large sample size ($n = 2{,}265$) makes the directional pattern robust despite the small magnitude.


\subsection{Robustness to Defection (RQ4)}

To evaluate whether cooperative training preserves negotiation effectiveness against adversarial counterparts, the LA-GRPO fair-only model is tested against four adversarial opponent personas, each implemented as a system-prompt variant of the base model. The four personas represent distinct manipulation strategies:
...
To assess whether cooperative negotiation training degrades general LLM capabilities, the GRPO all-equal model (checkpoint 620) and LA-GRPO all-equal model (checkpoint 2000) are evaluated on three standard benchmarks alongside the base Qwen3-14B-Instruct model. Evaluations are conducted using the Language Model Evaluation Harness with a vLLM backend on an A100 80\,GB GPU.

\begin{table}[H]
\centering
\caption[Benchmark results for base model vs.\ trained models.]{Benchmark results for base model vs.\ trained models. $\Delta$ shows the difference in percentage points vs.\ the base model.}
...
\begin{table}[H]
\centering
\caption[GRPO vs.\ LA-GRPO at matched compute budget.]{GRPO vs.\ LA-GRPO at matched compute budget (GRPO ckpt 560, LA-GRPO ckpt 760). Out-of-sample evaluation on 18 held-out game configurations, 360 negotiations per model. Bold indicates better per row within each $\lambda$ pair.}
...
\paragraph{RQ3 (Metrics).} The evaluation uses multiple complementary metrics rather than a single score. \textbf{Agreement rate} measures whether deals are reached, \textbf{agent payoff} ($U_A$) measures individual success, and the \textbf{Nash product} ($U_A \times U_B$) captures both efficiency and fairness in one number, penalizing lopsided outcomes more strongly than welfare ($U_A + U_B$) does. All ratio metrics are game-normalized by dividing achieved values by the per-game maximum, which is essential for comparing across heterogeneous games. Separating all-game ratios (failed negotiations contribute zero) from agreed-only ratios disentangles deal quantity from deal quality (Tables~\ref{tab:eval_results} and~\ref{tab:lagrpo_agreed}). Overall, the Nash product emerges as the best single composite cooperation metric, but agreement rate, agreed-only quality, and Pareto scatter plots (Section~\ref{sec:pareto_fairness}) remain essential for the full picture.

\paragraph{RQ4 (Robustness).} Cooperative training does not produce an exploitable agent against adversarial counterparts. The LA-GRPO fair-only model substantially outperforms the base model when facing hardball ($+30$ pp agreement, $+15.7$ on $U_A$), deceptive ($+15.7$ pp), and anchoring ($+22.1$ pp) opponents. The single exception is stubborn opponents, where both models struggle similarly (differences not statistically significant); however, when deals are reached, their quality is the highest of any tested cell.
...
\begin{enumerate}
    ...
    \item \textbf{Out-of-domain generalization} showing modest but measurable transfer of cooperative behavior to two unseen settings. On the Trust Game, fair-only as investor shows higher trust (send $5.48$ vs.\ $5.00$) and 31\% higher Nash product ($83.1$ vs.\ $63.4$). On GT-HarmBench ($2{,}265$ AI-safety scenarios across seven game-theoretic structures), GRPO fair-only achieves the highest utilitarian rate ($0.935$ overall, $0.881$ on Prisoner's Dilemma; $+1.8$ and $+2.9$ pp vs.\ base, respectively), while LA-GRPO self-only is the only configuration that performs \emph{worse} than the base---confirming that rewards with an opponent-aware component generalize positively, while pure self-interest generalizes negatively.
...
\begin{itemize}
    ...
    \item \textbf{Unmatched step counts between GRPO and LA-GRPO:} LA-GRPO was trained for 1340--2000 steps compared to 560--620 for GRPO. While LA-GRPO uses fewer tokens per step (shared prefix), the total token budgets were not precisely matched. Section~\ref{sec:matched_budget} attempts to address this with an out-of-sample matched-budget comparison, but a definitive resolution would require running both methods to convergence at multiple matched checkpoints, which exceeded the available training budget.
    ...
    \item \textbf{Limited benchmark coverage:} Only three benchmarks (MMLU-Pro, IFEval, GSM8K) were evaluated for capability preservation. Additional benchmarks measuring social, pragmatic, or emotional reasoning could reveal more subtle effects of cooperative training that are not captured by reasoning or instruction-following alone.
