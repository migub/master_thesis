...
\paragraph{Self-only fails to improve Nash product.} Self-only's Nash ratio plateaus near the base model's level, showing that pure self-interest provides no cooperative improvement over the untrained model. The agent extracts more for itself (higher $U_A$) but at the expense of the opponent---overall deal quality measured by Nash product remains unchanged. This pattern matches H2 (Section~\ref{sec:hypotheses}).
